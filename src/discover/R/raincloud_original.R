...
cohort_colors <- c("HCs" = "#1f77b4", "Class 2" = "#ff7f0e", "Class 3" = "#2ca02c", "Class 4" = "#d62728")

# Load U test results
u_test_results <- read_csv("src/discover/results/U_test_results/recon_dev_U_test_results.csv")
...
add_stat_annotations <- function(data, u_test_results, feature_name, plot_title) {
  ...
  data <- data %>%
    mutate(cohort = case_when(
      ...
      inter_test_subs == 1 ~ "Class 2",
      exter_test_subs == 1 ~ "Class 3",
      high_test_subs == 1 ~ "Class 4"
    ))

  # Ensure cohort order is HCs, Class 2, Class 3, Class 4
  data$cohort <- factor(data$cohort, levels = c("HCs", "Class 2", "Class 3", "Class 4"))
  ...
  p <- ggplot(data, aes(x = cohort, y = reconstruction_deviation, fill = cohort)) + 
    ...
    scale_fill_manual(values = cohort_colors) +  # Ensure the color mapping is correct
    annotate("text", x = 2, y = max_stats$y_max[max_stats$cohort == "Class 2"], label = inter_fdr_star, size = 8, color = "black") +  # PI vs HC
    annotate("text", x = 3, y = max_stats$y_max[max_stats$cohort == "Class 3"], label = exter_fdr_star, size = 8, color = "black") +  # PE vs HC
    annotate("text", x = 4, y = max_stats$y_max[max_stats$cohort == "Class 4"], label = high_fdr_star, size = 8, color = "black")  # HD vs HC

  return(p)
}

# Read the datasets
data_thickness <- read.csv("src/discover/results/output_data/t1w_cortical_thickness_rois_output_data_with_dev.csv")
data_volume <- read.csv("src/discover/results/output_data/t1w_cortical_volume_rois_output_data_with_dev.csv")
data_surface_area <- read.csv("src/discover/results/output_data/t1w_cortical_surface_area_rois_output_data_with_dev.csv")
...
ggsave("src/discover/R/images/original_cortical_thickness_distribution_with_stats.png", plot = plot_thickness, width = 12, height = 8, dpi = 300)
ggsave("src/discover/R/images/original_cortical_volume_distribution_with_stats.png", plot = plot_volume, width = 12, height = 8, dpi = 300)
ggsave("src/discover/R/images/original_cortical_surface_area_distribution_with_stats.png", plot = plot_surface_area, width = 12, height = 8, dpi = 300)
