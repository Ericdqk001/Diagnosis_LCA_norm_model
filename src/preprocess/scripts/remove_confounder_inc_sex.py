...
def deconfound_image_inc_sex():
    ...
    # Join the covariates to the cortical thickness data and cortical volume data
    ...
    for img_feature in t1w_cortical_features_list:
        ...
        formula = (
            "%s ~ C(demo_sex_v2) + smri_vol_scs_intracranialv + interview_age"
            % img_feature
        )
    ...
    cortical_features_resid_df_covars.to_csv(
        Path(processed_data_path, "t1w_cortical_features_resid.csv"),
        index=True,
    )

    rsfmri_resid_df_covars.to_csv(
        Path(processed_data_path, "gordon_cor_subcortical_resid.csv"),
        index=True,
    )
