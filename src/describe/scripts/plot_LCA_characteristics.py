...
for modality in feature_types:
    ...
    inter_subs = modality_data_split["internalising_test"]
    exter_subs = modality_data_split["externalising_test"]
    dysregulated_subs = modality_data_split["high_symptom_test"]

    if low_entropy:
        inter_subs = [sub for sub in inter_subs if sub not in high_entropy_subs]
        exter_subs = [sub for sub in exter_subs if sub not in high_entropy_subs]
        dysregulated_subs = [
            sub for sub in dysregulated_subs if sub not in high_entropy_subs
        ]

    sets = {
        "Predominantly Internalising": inter_subs,
        "Predominantly Externalising": exter_subs,
        "Highly Dysregulated": dysregulated_subs,
    }

    fig, axes = plt.subplots(3, 4, figsize=(20, 15))
    fig.suptitle(
        f"Distribution of Demographics of refined psychopathological classes for {modality.capitalize()} Modality",
        fontsize=16,
    )
